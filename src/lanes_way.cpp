#include "../include/lane_detector/lanes_way.hpp"
...
vector<double> gen_new_way(Mat img, double a, double lam1, double lam2, double w, vector<double> old_waypoint) {
    ...
    waypoint.push_back(old_waypoint[0] + JUMP*cos(old_waypoint[2]));
    waypoint.push_back(old_waypoint[1] + JUMP*sin(old_waypoint[2]));
    waypoint.push_back(old_waypoint[2]);
    return waypoint;
...
void Lanes::parabola()
{
 ...
 //    start = clock();
    ...
    */
	...
	//printing lanes
	/*
	double lane_score_l = 0, lane_score_r = 0;
	...
	for(int x = 0; x < top_view.cols && !flag_no_lane; x++)
	{
		...
		if (top_view_rgb.rows - y_l >= 0 && top_view_rgb.rows - y_l < dot.rows) lane_score_l++;
		if (top_view_rgb.rows - y_r >= 0 && top_view_rgb.rows - y_r < dot.rows) lane_score_r++;
		*/
...
/*
	cout << endl;
	cout << "lane_score_l: " << lane_score_l << "\nlane_score_r" << lane_score_r << endl;
	*/

/*
	double mario_score_l = 0;
	double mario_score_r = 0;
	for (int i = 0; i < mario.rows; i++) {

	    float y = mario.rows - i;
	    float xl = (y*y)/lam_gl + a_gl;
	    float xr = (y*y)/lam2_gl + a_gl + w_gl;

	    for (int j = -0; j < 1; j++) {
		if (xl+j >= 0 && xl+j < mario.cols && mario.at<uchar>(i, xl+j) == 255) mario_score_l++;
	    }
	    for (int j = -0; j < 1; j++) {
		if (xr+j >= 0 && xr+j < mario.cols && mario.at<uchar>(i, xr+j) == 255) mario_score_r++;
	    }
	}

	if (mario_score_l == 0) mario_score_l = 1;
	if (mario_score_r == 0) mario_score_r = 1;
	cout << "l_ratio " << lane_score_l/mario_score_l << endl;
	cout << "r_ratio " << lane_score_r/mario_score_r << endl;

	if (lane_score_l/mario_score_l >= extrapolation_factor) w_gl = 10000;
	if (lane_score_r/mario_score_r >= extrapolation_factor) w_gl = 10000;

	cout << "mario_score_l: " << mario_score_l << "\nmario_score_r" << mario_score_r << endl;
	cout << endl;
	...
	*/
	...
	obstacle_detected = frames_to_skip_when_obstacle_detected;
    }
